...
# It supports two agents, selected via the CLOMA_AGENT environment variable:
#
#   - claude (default): Claude Code, driven by the Anthropic Messages API.
#   - grok:             Grok Build, driven by an OpenAI-compatible endpoint.
#
# Both agents are pointed at an Ollama instance running on the host. Ollama is
...
# Make sure the per-user agent install location is on PATH (grok installs to ~/.grok/bin).
...
ensure_agent_installed() {
  case "${CLOMA_AGENT}" in
    ...
    grok)
      ...
      ;;
    *)
      log_error "Unknown agent: ${CLOMA_AGENT} (expected 'claude' or 'grok')"
      exit 1
      ;;
  esac
}
